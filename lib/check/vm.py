...
class CheckVm(Check):
    ...
    async def run(asset: Asset, local_config: dict, config: dict) -> dict:
        ...
        if balloon is not None and balloon.get('free_mem') is not None:
            state['ballooninfo'] = [{
                'name': 'ballooninfo',
                ...
                'percent_used':
                to_percent_used(balloon.get('total_mem'),
                                balloon.get('free_mem')),
                'percent_used_actual':
                to_percent_used(balloon.get('actual'),
                                balloon.get('free_mem'))
            }]
        if blockstat is not None:
            state['blockstat'] = [{
                ...
                'failed_flush_operations': n.get(
                    'failed_flush_operations'),  # int
                'failed_rd_operations': n.get('failed_rd_operations'),  # int
                'failed_unmap_operations': n.get(
        ...
        if support is not None:
            state['proxmox_support'] = [{
                ...
                'backup_max_workers': support.get(
                ...
                'pbs_library_version': support.get(
